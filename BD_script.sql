...
insert into producto (descripcion,categoria,precio) values ('LaptopL FRANKFURTER','CONDIMENTOS',13);

select * from producto where lower(descripcion) LIKE '%laptop%';
...
SELECT ID, IDVENTA, (SELECT DESCRIPCION FROM PRODUCTO WHERE ID=IDPRODUCTO) AS PRODUCTO_NOMBRE,
		(SELECT PRECIO FROM PRODUCTO WHERE ID=IDPRODUCTO) AS PRECIO_PRODUCTO, CANTIDAD,
        CANTIDAD * (SELECT PRECIO FROM PRODUCTO WHERE ID=IDPRODUCTO) AS TOTAL
FROM DETALLE;